...
let
  ...
  generateTargetConfigs = targets: 
    mapAttrs (name: target: {
      # Use the target name as the attribute key, and define the file content
      # and destination within the attribute's value.
      "${name}" = {
        target = "${target.directory}/${target.fileName}";
